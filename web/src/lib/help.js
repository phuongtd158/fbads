...
export const TOPICS = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'table', head: ['Ô số liệu', 'Ý nghĩa'], rows: [
        ['Chi tiêu', 'Tổng tiền đã tiêu trong khoảng ngày đang chọn (mặc định là hôm nay) của các camp đang xem. Khi xem hôm nay, vòng tròn cho biết đã dùng bao nhiêu % tổng ngân sách ngày của các camp đang chạy; các khoảng khác hiện trung bình mỗi ngày.'],
        ['Đang chạy', 'Số camp đang chạy trên tổng số camp.'],
        ['Kết quả', 'Số kết quả trong khoảng ngày đang chọn theo loại bạn chọn ở Cài đặt → Chung (đơn hàng, khách để lại thông tin, tin nhắn…).'],
      ...
      { t: 'steps', title: 'Bạn có thể làm gì', items: [
        ...
        '**Xem theo ngày:** nút ngày ở đầu trang (giống Ads Manager) cho chọn Hôm nay, Hôm qua, 7 / 14 / 28 / 30 ngày qua, Tuần này, Tháng này, Tháng trước, Tối đa… hoặc bấm 2 ngày trên lịch để chọn khoảng tuỳ ý (tối đa khoảng 37 tháng gần nhất). Các thẻ chỉ số và bảng đổi theo khoảng đã chọn; lựa chọn được nhớ cho lần sau. Ngân sách và trạng thái luôn là hiện tại. Các mốc “n ngày qua” **không tính hôm nay**, giống Ads Manager.',
        '**Tìm và lọc:** gõ vào ô tìm kiếm (phím tắt `/`, tìm được cả theo tên tài khoản), lọc Tất cả / Đang chạy / Không chạy, và chuyển giữa Chiến dịch / Nhóm QC. Bộ lọc đang áp dụng hiện thành các nhãn phía trên bảng, bấm × để bỏ từng cái hoặc “Xoá bộ lọc” để bỏ hết.',
        '**Chọn cột:** nút **Cột** bật thêm **Doanh thu** (giá trị chuyển đổi của loại kết quả bạn chọn ở Cài đặt → Chung, ví dụ tổng giá trị đơn hàng; để “–” hoặc 0 nếu chiến dịch không có giá trị), Lượt hiển thị, Lượt nhấp, CTR, CPC, CPM, cùng các cột kết quả phụ (không phụ thuộc loại kết quả đã chọn): Lượt bắt đầu cuộc trò chuyện, Chi phí/cuộc trò chuyện, Lượt bắt đầu thanh toán, Khách hàng tiềm năng, Khách hàng tiềm năng trên Meta, Bình luận về bài viết (mặc định 5 cột). **Đổi độ rộng cột:** kéo mép phải của tiêu đề cột; bấm đúp vào mép đó để cột về độ rộng tự động, hoặc bấm “Đặt lại độ rộng cột” trong nút **Cột** để đặt lại tất cả. Lựa chọn được nhớ. Nhiều cột thì bảng cuộn ngang, hai cột đầu, dòng tiêu đề và **hàng Tổng** ở đáy luôn cố định.',
      ...
      { t: 'note', tone: 'info', text: 'Mặc định số liệu là của **hôm nay** theo múi giờ tài khoản quảng cáo, có thể trễ vài phút so với Ads Manager. Khoảng ngày dài (nhất là Tối đa) tải lâu hơn và tốn nhiều lượt gọi Facebook hơn; tool dùng lại số vừa tải trong vài phút.' },
    ],
  },
  {
    ...
    blocks: [
      ...
      { t: 'steps', title: 'Tạo lịch', items: [
        ...
        'Chọn **hành động**: Bật camp, Tắt camp, **Bật + tắt theo giờ** (một lịch cho cả giờ bật lẫn giờ tắt), hoặc Đổi ngân sách.',
        'Đặt **giờ chạy** và **các ngày** trong tuần (có nút chọn nhanh Hằng ngày, T2–T6, Cuối tuần).',
        'Chọn **các chiến dịch** áp dụng rồi bấm **Lưu lịch**. Lọc theo **Trạng thái** (đang chạy / đang tắt) và **tên** (nhiều từ khoá cách nhau bằng dấu phẩy); mục đã chọn hiện thành thẻ phía trên danh sách, mỗi dòng có nhãn các lịch khác đang tác động lên nó.',
      ] },
      ...
      { t: 'tips', items: [
        ...
        'Bật camp mà mọi nhóm QC bên trong đang tắt thì camp vẫn không chạy: hộp thoại sẽ cảnh báo, hãy chọn thêm nhóm QC cần bật.',
        'Đã có lịch bật? Bấm **Tạo lịch tắt** trên thẻ lịch để tạo lịch tắt cho đúng các camp đó.',
      ] },
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'table', head: ['Ô trong form', 'Ý nghĩa'], rows: [
        ...
        ['Nếu (số liệu)', 'Chọn CPA, ROAS, Chi tiêu, Số kết quả, CTR, CPC, CPM, **Số tin nhắn / Chi phí mỗi tin nhắn** hoặc **Số lead / Chi phí mỗi lead** hoặc **Tần suất** (số lần trung bình mỗi người đã xem), rồi “lớn hơn” hoặc “nhỏ hơn” một giá trị. Bấm **Thêm điều kiện** (tối đa 5) rồi chọn **VÀ** (tất cả đều đúng) hoặc **HOẶC** (một trong số đó đúng). Ví dụ: CPA cao **VÀ** ROAS thấp mới tắt.'],
        ...
        ['Áp dụng cho', 'Chọn cấp **Chiến dịch** hoặc **Nhóm QC**, rồi áp dụng cho tất cả mục đang chạy ở cấp đó hoặc chọn từng mục cụ thể. Khi chọn “tất cả” và có nhiều tài khoản, có thể giới hạn **Trong tài khoản** nào. Ngân sách chỉ nằm ở một cấp: chiến dịch **CBO** giữ ngân sách ở chiến dịch, chiến dịch **ABO** giữ ở từng nhóm QC. Rule tăng/giảm ngân sách tự bỏ qua mục không có ngân sách riêng (Xem trước ghi rõ lý do); rule tắt và rule thông báo áp dụng cho mọi mục.'],
      ] },
      { t: 'example', title: 'Ví dụ rule hay dùng', items: [
        ...
        '**Cháy quảng cáo:** nếu tần suất 7 ngày lớn hơn 3,5 và CTR nhỏ hơn 1% thì gửi cảnh báo để thay nội dung.',
        '**Bộ rule theo mục tiêu:** bấm **Bộ rule theo mục tiêu**, chọn Bán hàng / Tin nhắn / Lead, nhập chi phí mục tiêu một lần là tạo sẵn 4 rule: cắt lỗ trong ngày, tắt camp đắt, tăng ngân sách camp tốt, cảnh báo cháy quảng cáo.',
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'p', text: 'Mỗi dòng ghi **tên camp/nhóm QC** trước, kết quả ở dưới, và lịch/rule gây ra nó ở dòng nhãn nhỏ. Khi một lịch hoặc rule chạy một lượt ảnh hưởng **từ 3 mục trở lên**, các dòng được **gộp thành một khối** (vd “Rule “Tăng” · 14 mục · 14 cảnh báo”); bấm vào khối để xem từng mục. Lọc theo kết quả (Thành công, Cảnh báo, Chạy thử, Bỏ qua, Lỗi), theo nguồn, hoặc gõ tìm tên camp, rule, mã lỗi. Khi đang tìm, các khối tự mở sẵn. Trang tự tải lại khi bạn quay lại tab.' },
      ...
      { t: 'steps', title: 'Xem chi tiết một dòng', items: [
        'Bấm vào dòng nhật ký (với khối gộp: bấm mở khối rồi bấm vào mục bên trong).',
...
export const TIPS = {
  ...
  scheduleWindow: { text: 'Một lịch làm 2 việc: bật các mục lúc giờ bật và tắt chúng lúc giờ tắt. Giờ tắt sớm hơn giờ bật nghĩa là tắt sau nửa đêm (sáng hôm sau). Nút Chạy ngay đưa các mục về đúng trạng thái của khung giờ lúc bấm.', topic: 'schedules' },
