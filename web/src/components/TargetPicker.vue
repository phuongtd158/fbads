...
// Chỉ VẼ một phần danh sách (vài nghìn mục thì vẽ hết sẽ giật). Chọn hết vẫn chọn toàn bộ danh sách đang lọc.
const pageSize = () => (window.matchMedia('(max-width: 640px)').matches ? 40 : 100)
const cap = ref(pageSize())
const shown = computed(() => (list.value.length > cap.value ? list.value.slice(0, cap.value) : list.value))
const moreItems = computed(() => Math.min(pageSize(), list.value.length - shown.value.length))
watch([q, account, () => props.level], () => { cap.value = pageSize() })
...
.morelnk { display: block; width: 100%; padding: 10px 12px; margin-top: 4px; border: 0; border-radius: 9px; background: var(--surface-2); color: var(--accent); font: inherit; font-size: 13px; font-weight: 650; cursor: pointer; }
