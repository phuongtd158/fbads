...
function metricsFrom(row, resultAction) {
  ...
  const results = pick(row.actions, names);
  const value = pick(row.action_values, names);
  return {
    spend,
    impressions: parseInt(row.impressions || 0, 10),
    reach: parseInt(row.reach || 0, 10), // số người đã xem (không trùng) → tần suất = hiển thị / reach
    ...
    // Các cột "kết quả phụ" độc lập với loại kết quả đã chọn ở Cài đặt — giống các cột riêng lẻ của Ads Manager.
    conversations: pick(row.actions, ['onsite_conversion.messaging_conversation_started_7d']),
    checkouts: pick(row.actions, RESULT_ALIASES.initiate_checkout),
    leads: pick(row.actions, RESULT_ALIASES.lead),
    leadsOnMeta: pick(row.actions, ['onsite_conversion.lead_grouped']),
    comments: pick(row.actions, ['comment']),
  };
}

const EMPTY = { spend: 0, impressions: 0, reach: 0, clicks: 0, results: 0, revenue: 0, conversations: 0, checkouts: 0, leads: 0, leadsOnMeta: 0, comments: 0, cpa: null, roas: null };
...
async function listAccount(id, resultAction) {
  ...
  const [camps, adsets, ic, ia] = await Promise.all([
    ...
    graphAll(`${act}/insights`, { level: 'campaign', date_preset: 'today', fields: 'campaign_id,spend,impressions,reach,clicks,actions,action_values' }),
    graphAll(`${act}/insights`, { level: 'adset', date_preset: 'today', fields: 'adset_id,spend,impressions,reach,clicks,actions,action_values' }),
  ]);
...
// Số "kết quả phụ" giả (cuộc trò chuyện, bắt đầu thanh toán, khách hàng tiềm năng, bình luận) theo chi tiêu và seed của camp mẫu
function mockExtras(spend, seed) {
  return {
    conversations: Math.floor(spend / (70000 + (seed % 3) * 20000)),
    checkouts: Math.floor(spend / (130000 + (seed % 4) * 30000)),
    leads: Math.floor(spend / (90000 + (seed % 3) * 25000)),
    leadsOnMeta: Math.floor(spend / (110000 + (seed % 3) * 25000)),
    comments: Math.floor(spend / (40000 + (seed % 4) * 15000)),
  };
}
function mockList() {
  ...
  return mockObjs.map((o) => {
    ...
    return { ...o, metrics: { spend, impressions: spend * 9, reach: Math.round((spend * 9) / (1.3 + (o.seed % 4) * 0.9)), clicks: Math.round(spend / 900), results, revenue: results * 380000,
      cpa: results ? spend / results : null, roas: spend ? (results * 380000) / spend : null, ...mockExtras(spend, o.seed) } };
  });
}
...
function mockRange(days) {
  ...
  return Object.fromEntries(mockObjs.map((o) => {
    ...
    return [o.id, { spend, impressions: spend * 9, reach: Math.round((spend * 9) / (1.3 + (o.seed % 4) * 0.9)), clicks: Math.round(spend / 900), results, revenue: results * 380000, cpa: results ? spend / results : null, roas: spend ? (results * 380000) / spend : null, ...mockExtras(spend, o.seed) }];
  }));
}
  ...
  for (const id of accountIds()) {
    try {
      const [ic, ia] = await Promise.all([
        graphAll(`${actOf(id)}/insights`, { level: 'campaign', ...fbParams, fields: 'campaign_id,spend,impressions,reach,clicks,actions,action_values' }),
        graphAll(`${actOf(id)}/insights`, { level: 'adset', ...fbParams, fields: 'adset_id,spend,impressions,reach,clicks,actions,action_values' }),
      ]);
