...
async function realList() {
  ...
  const [camps, adsets, ic, ia] = await Promise.all([
    ...
    graphAll(`${acct()}/adsets`, { fields: 'id,name,status,effective_status,daily_budget,campaign_id,learning_stage_info' })
      .catch(() => graphAll(`${acct()}/adsets`, { fields: 'id,name,status,effective_status,daily_budget,campaign_id' })),
  ...
  return [
    ...
    ...adsets.map((a) => ({ id: a.id, name: a.name, level: 'adset', campaignId: a.campaign_id, status: a.status, effective: a.effective_status, dailyBudget: conv(a.daily_budget), learning: isLearning(a), metrics: ma[a.id] || EMPTY })),
  ];
}
...
const resetCache = () => { cache = { at: 0, data: null }; for (const k of Object.keys(rangeCache)) delete rangeCache[k]; };
...
module.exports = { resetMock, rangeMetrics, snapshot, describeError, resetCache, peekObjects, listObjects, setStatus, setBudget, testConnection, listAccounts, extendToken, inspectToken, getCurrency: () => currency };
