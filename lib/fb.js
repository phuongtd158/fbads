...
async function graph(method, path, params = {}, tokenOverride) {
  ...
  try {
    ...
    throw new Error('Không kết nối được tới Facebook. Kiểm tra mạng internet.');
  }
  const json = await res.json();
  if (json.error) throw new Error(friendly(json.error));
  return json;
}
...
module.exports = { listObjects, setStatus, setBudget, testConnection, listAccounts, extendToken, inspectToken, getCurrency: () => currency };
