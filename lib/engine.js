...
const METRIC_LABEL = { cpa: 'CPA', roas: 'ROAS', spend: 'Chi tiêu', results: 'Kết quả', ctr: 'CTR', cpc: 'CPC', cpm: 'CPM' };
...
const showValue = (v, metric) => (v === Infinity ? '∞ (chưa có kết quả)' : metric === 'roas' ? Number(v).toFixed(2) : metric === 'ctr' ? `${Number(v).toFixed(2)}%` : money(v));
...
function thresholdOf(c, obj) {
  ...
  const t = Number(((store.get().settings.accountTargets || {})[obj.accountId] || {})[c.metric]);
  return t > 0 ? (t * (Number(c.factor) || 100)) / 100 : null;
}
const fmtTh = (c) => (c.metric === 'roas' ? Number(c.threshold).toFixed(2) : c.metric === 'ctr' ? `${Number(c.threshold).toFixed(2)}%` : money(c.threshold));
const condSentence = (c, range) => `${METRIC_LABEL[c.metric]} ${RANGE_LABEL[range]} = ${showValue(c.actual, c.metric)} ${c.op === '>' ? 'lớn hơn' : 'nhỏ hơn'} ngưỡng ${fmtTh(c)}${c.vs === 'target' ? ` (${c.factor}% mục tiêu)` : ''}`;
...
function evaluateRule(rule, objs, mapFor, opts = {}) {
  ...
  const list = rule.allActive ? objs.filter((o) => o.level === level && o.effective === 'ACTIVE' && inAccounts(o)) : objs.filter((o) => (rule.targets || []).includes(o.id));
...
async function runRules() {
  ...
  for (const rule of rules) {
    for (const d of evaluateRule(rule, objs, (r) => maps[r])) {
      ...
      await act(d.obj, d.action, `Rule: ${rule.name} [${what}]`, { kind: 'rule', refId: rule.id, refName: rule.name, condition });
    }
  }
}
...
module.exports = { start, tickSchedules, runSchedule, runRules, sendReport, previewRule, undoLog, checkKillSwitch, plan, evaluateRule, act, metricValue };
