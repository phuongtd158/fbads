...
const METRIC_LABEL = { cpa: 'CPA', roas: 'ROAS', spend: 'Chi tiêu', results: 'Kết quả' };
const RANGE_LABEL = { today: 'hôm nay', yesterday: 'hôm qua', last_3d: '3 ngày gần nhất', last_7d: '7 ngày gần nhất' };
const EMPTY_M = { spend: 0, results: 0, cpa: null, roas: null };
...
function plan(obj, action, ctx = {}) {
  ...
  if (action.type === 'on' || action.type === 'off') {
    ...
    return { kind: 'do', detail: want ? 'Bật camp' : 'Tắt camp', after: { status: want ? 'ACTIVE' : 'PAUSED' } };
  }
  if (obj.dailyBudget == null) return { kind: 'error', message: 'Đối tượng này không có ngân sách ngày (có thể đang dùng CBO ở cấp camp)' };
...
async function act(obj, action, source, ctx = {}) {
  ...
  if (p.kind === 'skip') { // ghi nhận việc bỏ qua, mỗi lý do chỉ 1 lần/ngày/camp để khỏi đầy nhật ký; không gửi Telegram
    ...
    if (day.skipLogged[key]) return;
    day.skipLogged[key] = 1; store.save();
    await record({ ...base, detail: `Bỏ qua: ${p.reason}`, ok: true, dry, skipped: true, silent: true });
    return;
  }
  if (p.kind === 'error') { await record({ ...base, detail: p.message, ok: false, dry, error: { message: p.message }, silent: ctx.silent }); return; }
  try {
    if (!isNotify && !dry) {
...
const showValue = (v, metric) => (v === Infinity ? '∞ (chưa có kết quả)' : metric === 'roas' ? Number(v).toFixed(2) : money(v));

function actionOf(rule) {
  if (rule.action === 'pause') return { type: 'off' };
  if (rule.action === 'notify') return { type: 'notify' };
  return { type: 'budget', mode: 'percent', value: rule.action === 'increase' ? rule.pct : -rule.pct, max: rule.maxBudget, min: rule.minBudget };
}
...
function evaluateRule(rule, objs, mapFor, opts = {}) {
  ...
  const list = rule.allActive ? objs.filter((o) => o.level === level && o.effective === 'ACTIVE') : objs.filter((o) => (rule.targets || []).includes(o.id));
  ...
  return list.map((obj) => {
    ...
    if (obj.effective !== 'ACTIVE') return Object.assign(d, { status: 'skip', code: 'inactive', reason: 'Camp không đang chạy' });
    ...
    d.value = metricValue(metrics, rule.metric);
    d.hit = rule.op === '>' ? d.value > rule.value : d.value < rule.value;
    if (!d.hit) return d;
    ...
    const act_ = action.type === 'notify'
      ? { ...action, message: `${METRIC_LABEL[rule.metric]} ${RANGE_LABEL[range]} = ${showValue(d.value, rule.metric)} ${rule.op === '>' ? 'lớn hơn' : 'nhỏ hơn'} ngưỡng ${rule.metric === 'roas' ? rule.value : money(rule.value)}` }
      : action;
    d.action = act_;
...
async function runRules() {
  ...
  for (const rule of rules) {
    for (const d of evaluateRule(rule, objs, (r) => maps[r])) {
      ...
      const condition = { metric: rule.metric, op: rule.op, range: d.range, threshold: rule.value, actual: Number.isFinite(d.value) ? d.value : null, actualInf: d.value === Infinity, minSpend: rule.minSpend || 0, spend: d.metrics.spend, cooldownHours: rule.cooldownHours || 0 };
      await act(d.obj, d.action, `Rule: ${rule.name} [${METRIC_LABEL[rule.metric]}${range} ${showValue(d.value, rule.metric)}]`, { kind: 'rule', refId: rule.id, refName: rule.name, condition });
    }
  }
}
...
async function previewRule(rule) {
  ...
  const items = evaluateRule(rule, objs, () => map).map((d) => ({
...
async function checkKillSwitch(objs) {
  ...
  const limit = Number(s.dailySpendLimit) || 0;
  if (!s.killSwitchEnabled || limit <= 0) return false;
  const today = localNow().date;
  if (st.killFired === today) return false;
  ...
  await record({
    ...
    detail: `Chi tiêu ${money(spend)} đã vượt mức ${money(limit)} — ${dry ? '(chạy thử) sẽ tắt' : 'tắt'} ${active.length} camp đang chạy.`,
...
module.exports = { start, runSchedule, runRules, sendReport, previewRule, undoLog, checkKillSwitch, plan, evaluateRule, act };
